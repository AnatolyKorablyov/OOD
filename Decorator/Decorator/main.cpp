...
{
	{
		...
		auto latte = make_unique<CLatte>(SizeBeverage::Standart);
		// добавляем корицы
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CLatte>(SizeBeverage::Double)),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		auto tea = make_unique<CTea>(TeaType::BlackTea);
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CTea>(TeaType::RedTea)	// Берем чай
	...
	{
		auto beverage =
			make_unique<CMilkshake>(ServingSize::Big)					// Наливаем молочный коктейль
